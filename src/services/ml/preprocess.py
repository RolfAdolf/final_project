...
def scaler_return(train: np.ndarray):
    scaler = StandardScaler()
    scaler.fit(train)
    return scaler


def train_return(train_data: PandasDataFrame, normalizing: bool = True) -> Dict:
    ...
    if normalizing:
        ...
        scaler = None
    return {"data": (x, y), "scaler": scaler}


def train_test_return(
    train: PandasDataFrame, test_size_proportion: float, normalizing: bool = True
) -> Dict:
    x, y = feature_target_return(train)
    x_train, x_test, y_train, y_test = train_test_split(
        x, y, stratify=y, test_size=test_size_proportion
    )
    if normalizing:
...
def test_return(test: PandasDataFrame) -> np.ndarray:
