...
from fastapi import APIRouter, Depends, HTTPException, status, BackgroundTasks, UploadFile, Form, File
...
def get_all(ml_service: MLService = Depends(), user_role: str = Depends(get_current_user_role)):
    """
    Получить все модели.
    """
...
def get_mine(ml_service: MLService = Depends(), user_id: str = Depends(get_current_user_id)):
    """
...
def get_data(
    ...
    return StreamingResponse(download_file, media_type='text/csv',
                             headers={'Content-Disposition': 'attachment; filename=predictive_maintenance.csv'})
