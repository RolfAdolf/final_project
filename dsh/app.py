...
def display_page(pathname, user_data):
    ...
    if not (user_data.get("username", False) and user_data.get("password", False)):
        ...
        try:
            ...
            username, role = (
                authorization_result["username"],
                authorization_result["role"],
            )
...
@app.callback(
    Output("preprocess_reference", "children"), Input("preprocess_button", "n_clicks")
)
...
@app.callback(
    Output("preprocess_reference", "children"), Input("back_button", "n_clicks")
)
...
def download_preprocessed_data(file: Optional[str], filename: str, user_data: Dict):
    if file is None:
        raise PreventUpdate()
    try:
        preprocessed_data, graph = preprocess_data(
            file, filename, user_data["username"], user_data["password"]
        )
        style = dict(preprocess.style_graph)
        style["display"] = "block"
        return (
            dcc.send_data_frame(preprocessed_data, "preprocessed_data.csv"),
            style,
            graph,
        )
    except requests.exceptions.HTTPError:
...
@app.callback(Output("train_reference", "children"), Input("train_button", "n_clicks"))
def back_from_preprocess(n_clicks: int):
    if n_clicks is None:
        raise PreventUpdate()
...
def train_model_menu(file: str, filename: str, model_type: str, user_data: Dict):
    ...
    try:
        train_model_request(
            file, filename, model_type, user_data["username"], user_data["password"]
        )
    except requests.exceptions.HTTPError:
...
@app.callback(
    Output("predict_reference", "children"), Input("predict_button", "n_clicks")
)
def back_from_preprocess(n_clicks: int):
    if n_clicks is None:
        raise PreventUpdate()
